...
/// "Symbol / Description / Equation / Value / Units" style as the AGMA 929-B22 annex
/// example tables, so the whole audit trail can be dumped to the results text box.
/// </summary>
public class CalcTrace
{
    ...
    public double R(string symbol, string equation, string description, double value, string unit = "mm")
    {
    ...
    {
        // Column widths are derived from the actual data (with sane minimums) rather than
        // fixed constants, so a longer label (e.g. in the closing summary section) always
        // gets at least one space of separation instead of running into the next column.
